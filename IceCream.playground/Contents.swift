...
func namesForIceCreamFlavor(flavor: String) -> [String] {
    var likesThisFlavor: [String] = []
    for favorite in favoriteFlavorsOfIceCream {
        if flavor == favorite.1 {
            likesThisFlavor.append(favorite.0)
        }
    }
     return likesThisFlavor
}
...
func countForIceCreamFlavor(flavor: String) -> Int {
    var finalCount = 0
    for favorite in favoriteFlavorsOfIceCream.values {
        if flavor == favorite {
            finalCount += 1
        }
    }
...
func removePerson(name: String) -> Bool {
    ...
    for person in favoriteFlavorsOfIceCream.keys {
        if name == person {
